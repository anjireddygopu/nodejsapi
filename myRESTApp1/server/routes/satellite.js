...
var satellite= { 
     getAll:async (req, res)=> {
       try{
       const results=await  pool.query('select  facility_center_id, facility_center_name, contact_person_name, contact_no1,email_id ,active from  tm_satellite_center',);
     ...
     getOne:async (req, res)=> {
      try{
      const results=await  pool.query('select  * from tm_satellite_center where facility_center_id=?', [req.params.id]);
        res.end(JSON.stringify(results));
      }
      catch(err) {
        throw new Error(err);
   ...
   const results=await pool.query('INSERT INTO tm_satellite_center set ?',postData);
        res.end(JSON.stringify(results));
        }
        catch(err) {
          throw new Error(err);
